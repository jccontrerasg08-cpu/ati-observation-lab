...
The application accepts observations only from a configured trusted proxy. The proxy must remove visitor-address headers, derive an opaque HMAC client pseudonym at the edge, and send that value together with a private origin token. For controlled `/lab/*` navigation, the Worker additionally derives an opaque session pseudonym and forwards it as `X-ATI-Proxy-Session-ID`; the raw session token is never sent to Railway. The application rejects direct requests and invalid proxy context with `503`; invalid or missing laboratory session context receives `403`. It never falls back to `X-Forwarded-For`, `X-Real-IP`, the immediate Railway peer, a cookie, or a client-supplied proxy session identifier.
...
> The edge pseudonyms support controlled rate limiting and grouped evaluation. A campaign marker proves that a request was generated by an authorized experiment, not the identity or intention of an external party.
...
3. Start each navigation session at `/lab/start` through the Worker hostname, then execute a manifest-defined sequence across the closed catalogue. Carry `X-ATI-Lab-Session` only in that session’s later `/lab/*` requests; do not put it in a cookie or a URL.
